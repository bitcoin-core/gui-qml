...
void ActivityListModelTests::liveInsertMatchesRefreshOrder()
{
    ...
    // The application of a notification is queued onto the model's thread,
    // so give the event loop a turn before reading the rows.
...
void ActivityListModelTests::proxyOrderMatchesSourceForTiedTimestamps()
{
    ...
    for (const bool ascending_arrival : {true, false}) {
        ...
        // Notification applications are queued onto the model's thread.
