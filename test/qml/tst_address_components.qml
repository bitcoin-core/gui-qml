...
TestCase {
    ...
    function test_row_uses_display_amount_and_emits_actions() {
        ...
        compare(addressLabel.width, Math.min(addressLabel.naturalWidth, addressLabel.parent.width))
        verify(addressLabel.width < addressLabel.parent.width)
    ...
    function test_addressList_doesNotReuseFocusedEditableRows() {
        const page = createTemporaryObject(addressListComponent, host)
        verify(page !== null)

        page.addressModel.resetForTest()
        page.addressModel.setAddressLabelSucceeds = true
        ...
        const firstAddress = page.addressModel.addressAt(0)
        const firstRow = list.itemAtIndex(0)
        const firstNote = findObject(firstRow, "addressRowNoteField")
        verify(firstNote !== null)
        firstNote.forceActiveFocus()
        tryCompare(firstNote, "activeFocus", true)
        firstNote.text = "Draft for the first address"
        firstNote.textEdited()

        const targetIndex = 10
        const targetAddress = page.addressModel.addressAt(targetIndex)
        const targetLabel = page.addressModel.labelAt(targetIndex)
        verify(firstAddress !== targetAddress)
        compare(list.itemAtIndex(targetIndex), null)

        list.positionViewAtIndex(targetIndex, ListView.Beginning)
        list.forceLayout()
        tryVerify(function() { return list.itemAtIndex(targetIndex) !== null })

        const targetRow = list.itemAtIndex(targetIndex)
        const targetNote = findObject(targetRow, "addressRowNoteField")
        verify(targetNote !== null)
        targetNote.forceActiveFocus()
        tryCompare(targetNote, "activeFocus", true)

        compare(page.addressModel.labelAt(0), "Draft for the first address")
        compare(page.addressModel.labelAt(targetIndex), targetLabel)
        compare(list.reuseItems, false)

        page.addressModel.category = AddressListModel.Change
        tryCompare(list, "reuseItems", true)
        page.addressModel.category = AddressListModel.SingleUse
        page.addressModel.setAddressLabelSucceeds = false
    }
    ...
    function test_details_hides_note_for_change_addresses() {
        const details = createTemporaryObject(detailsComponent, this, {
            category: "change"
        })
        verify(details !== null)
