...
def test_structured_columns_and_filters(gui):
    ...
    assert timestamp == expected_time, (
        f"Expected local HH:MM:SS time {expected_time!r}, got {timestamp!r}"
    )
    ...
    gui.wait_for_property("debugLogListView", "count", 1, timeout_ms=5000)
    gui.click("debugLogFilterButton")
    gui.wait_for_property("debugLogFilterMenu", "opened", True, timeout_ms=3000)
