...
void OnboardingProfileTests::invalidExplicitDataDirReturnsErrorWithoutReadingProfile()
{
    QTemporaryDir temp_dir;
    QVERIFY(temp_dir.isValid());
    // Leave the directory before QTemporaryDir removes it, including on Windows.
...
void OnboardingProfileTests::validRelativeExplicitDataDirResolvesAgainstWorkingDirectory()
{
    ...
    // The current directory can resolve a symlink in the temporary path,
    // such as /var -> /private/var on macOS.
    const QString absolute_data_dir{QDir::current().filePath(relative_data_dir)};
...
void OnboardingProfileTests::qmlOnboardedConfiguredDatadirProfileSkipsPreInitOnboarding_data()
{
...
void OnboardingProfileTests::onboardingApplyWithoutTouchedSettingsOnlyAddsQmlOnboardedMarker()
{
    ...
    check_args.LockSettings([&](common::Settings& settings) {
        has_listen_override = settings.rw_settings.contains("listen");
        has_prune_override = settings.rw_settings.contains("prune");
    });
...
void OnboardingProfileTests::onboardingApplyMigratesLegacySettingsBeforeTouchedOverrides()
{
    ...
    check_args.LockSettings([&](common::Settings& core_settings) {
        has_listen_override = core_settings.rw_settings.contains("listen");
...
void OnboardingProfileTests::onboardingApplyWritesTouchedParameterInteractionOverride()
{
    ...
    check_args.LockSettings([&](common::Settings& settings) {
        ...
        has_forced_listen = settings.forced_settings.contains("listen");
        has_forced_natpmp = settings.forced_settings.contains("natpmp");
        has_forced_discover = settings.forced_settings.contains("discover");
    });
...
void OnboardingProfileTests::onboardingApplyRetainsListenChoiceWhenDisablingProxy()
{
    ...
    check_args.LockSettings([&](common::Settings& settings) {
        ...
        has_proxy_override = settings.rw_settings.contains("proxy");
    });
    QVERIFY(listen_override.isBool());
    QVERIFY(!listen_override.get_bool());
