...
QByteArray TestBridge::input(const QString& object_name, const QString& action, const QString& text) const
{
    ...
    if (action == QLatin1String("click")) {
        // Newly loaded controls can still have pending layout updates. Polish
        // their ancestors first so the click uses the final scene coordinates.
        QList<QPointer<QQuickItem>> ancestors;
        for (QQuickItem* ancestor = item; ancestor; ancestor = ancestor->parentItem()) {
            if (ancestors.size() >= MAX_OBJECT_DEPTH) {
                return errorResponse(QStringLiteral("Input target exceeds automation depth limit"));
            }
            ancestors.prepend(ancestor);
        }
        for (const auto& ancestor : ancestors) {
            if (ancestor) ancestor->ensurePolished();
        }
        if (!item || !window || item->window() != window || !item->isVisible() || !item->isEnabled()) {
            return errorResponse(QStringLiteral("Input target became unavailable during layout"));
        }
